<?php
  ...
  //if the user clicks on make Reservation button 
  if (isset($_POST['makeReservation'])) {

    //lets check if there are empty fields
    if (empty($_POST['date']) || empty($_POST['hour']) || empty($_POST['seats'])) {
      echo '<script>alert("You have empty fields ! Please try again ")</script>';
      echo '<script>window.location.href="MakeReservation.php"</script>';
    } else {
      //let's verify if at the selected time by the user there are available seats 
      ...
      // select the resevations on the database with the same selected data by the user 
      ...
      if (($NumberofSeats + $seats) <= 20) {
        // if we still have available seats ; we make the reservation  
    ...
    <div class="make-reservation-left">
      ...
      <form class="form" method="POST" >
        ...
        <div class="val">
          ...
          <select class="form-input" name="hour" required>
        ...
        <div class="val">
          <label for="hour" >Select Table Seats</label>
          <select class="form-input" name="seats" type="number" required placeholder="">
        ...
        <div class="buttons">
          <button type="submit" name="makeReservation" class="button">Reserve</button>
          <button type="return" name="return" class="button">Return</button>
        </div>
      </form>
    </div>
  </div>
